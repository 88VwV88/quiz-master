...
<template>
  <div class="container">
    ...
    <div v-if="scores && scores.length !== 0">
      <div
        class="row text-center rounded bg-secondary lead text-white fw-bold p-2 mt-2"
      >
        <span class="col">Quiz ID</span>| <span class="col">Date</span>|
      ...
      <div v-for="(score, key) in scores" :key>
        <div class="row text-center rounded bg-dark p-2 mt-2">
          ...
          >|
          <span class="col">{{
            new Date(score.date_of_quiz).toISOString().split("T")[0]
          }}</span
          >|
    ...
    <div v-else class="lead bg-dark rounded p-2 text-center">
      No scores available
    </div>
  </div>
</template>
