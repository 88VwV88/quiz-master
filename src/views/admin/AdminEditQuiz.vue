...
function onSubmit() {
  store.dispatch("addSubject", subject).then(() => router.push("/admin"));
}
</script>
...
<template>
  <div class="mx-5 px-5">
    ...
    <form @submit.prevent.stop="onSubmit" class="container-md">
      <div
        v-for="(attr, key) in Object.keys(subject)"
        class="form-floating text-start my-2"
        :key
      >
        <textarea
          rows="10"
          cols="30"
          v-model="subject[attr]"
      ...
      <ul class="rounded bg-dark p-3">
        ...
        <li v-for="(chapter, i) in subject.chapters" :key="i" class="col-md">
          <div
            class="form-floating my-2 row-sm"
            v-for="attr in Object.keys(chapter)"
            :key="attr"
          >
            <textarea
              v-if="attr === 'description'"
              v-model="chapter[attr]"
              :id="`chapter-${i}-${attr}`"
              class="form-control"
              autocomplete="off"
            ></textarea>
            <input
              ...
              autocomplete="off"
            />
            <label :for="`chapter-${i}-name`"
              >Chapter {{ i + 1 }} {{ attr }}</label
            >
          </div>
          <button
            @click.prevent.stop="() => removeChapter(i)"
            class="btn btn-danger fs-5 my-2 w-100"
          >
            -
          </button>
        </li>
        <button
          @click.prevent.stop="addChapter"
