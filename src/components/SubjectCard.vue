  ...
  <div class="bg-secondary rounded p-3 card me-3 mt-3">
    ...
    <div class="row px-2 gap-2">
      <button
        ...
        @click="
          () => {
            store.dispatch('deleteSubject', subject.subject_id);
          }
        "
      >
    ...
    <div class="accrdn rounded mt-2" :id="`subject${subjectKey}-chapters`">
      ...
      >
        <h2 class="accrdn__header">
          <button
            class="btn btn-dark text-muted fs-6 lead"
            data-bs-toggle="collapse"
            type="button"
            :data-bs-target="`#subject${subjectKey}-chapter${i}`"
            :id="`header-${i}`"
          >
            {{ chapter.name }}
          </button>
        ...
        <div
          class="accrdn__body my-1"
          :data-bs-parent="`#subject${subjectKey}-chapter`"
        >
          <div
            :id="`subject${subjectKey}-chapter${i}`"
